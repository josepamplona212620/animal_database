...
def forward_propagation(X, parameters):
    ...
    Z2 = tf.nn.conv2d(A1,W2, strides = [1,1,1,1], padding = 'SAME') #64*64*16
    ...
    Z3 = tf.nn.conv2d(A2,W3, strides = [1,1,1,1], padding = 'SAME') #64*64*24
    ...
    P3 = tf.nn.max_pool(A3, ksize = [1,8,8,1], strides = [1,8,8,1], padding = 'SAME') #8*8*24
...
#    # 6 neurons in output layer. Hint: one of the arguments should be "activation_fn=None" 
    ...
    F2 = tf.contrib.layers.fully_connected(F1, 120, None)
    
    Z4 = tf.contrib.layers.fully_connected(F2, 6, None)
...
def model(X_train, Y_train, X_test, Y_test, learning_rate = 0.009, num_epochs = 100, minibatch_size = 64, print_cost = True):
    ...
    with tf.Session() as sess:
        ...
        save_path = saver.save(sess, "/home/madcas/Dropbox/ITM/Extensión/DeepLearning/Grupo02/Clase#4_RedesConvolucionales/model_softmax_Gestos4Capas_2fullConect.ckpt")
