...
X_train_orig, Y_train_orig, X_test_orig, Y_test_orig = load_dataset_cifar()      # Cargar la base de datos
...
def model(X_train, Y_train, X_test, Y_test, learning_rate = 0.009, num_epochs = 5, minibatch_size = 16, print_cost = True):
    ...
    with tf.Session() as sess:
        ...
        # Guardar la funcion de costo
        with open('C2FC1_cost.txt', 'w') as f:
            for item in costs:
                f.write("%s\n" % item)
        ...
        save_path = saver.save(sess, "/DATA/OneDrive - Instituto Tecnológico Metropolitano/Docs ITM/Diplomado Deep Learning/Clase - TensorFlow Redes Convloducionales/full/RedesConvolucionales_Clase1_full/model_softmax.ckpt")
