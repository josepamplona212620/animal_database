...
def forward_propagation(X, parameters):
    ...
    Z1 = tf.nn.conv2d(X, W1, strides = [1,1,1,1], padding = 'SAME')  #64*64*8
    ...
    P1 = tf.nn.max_pool(A1, ksize = [1,2,2,1], strides = [1,2,2,1], padding = 'SAME') #32*32*8
    
    # CONV2D: filters W2, stride 1, padding 'SAME'
    
    Z2 = tf.nn.conv2d(P1,W2, strides = [1,1,1,1], padding = 'SAME') #32*32*16
    ...
    P2 = tf.nn.max_pool(A2, ksize = [1,4,4,1], strides = [1,4,4,1], padding = 'SAME') #8*8*16
    
    # CONV2D: filters W3, stride 1, padding 'SAME'
    
    Z3 = tf.nn.conv2d(P2,W3, strides = [1,1,1,1], padding = 'SAME') #8*8*24
    ...
    P3 = tf.nn.max_pool(A3, ksize = [1,4,4,1], strides = [1,4,4,1], padding = 'SAME') #2*2*24
...
#    # 6 neurons in output layer. Hint: one of the arguments should be "activation_fn=None" 
    ...
    Z4 = tf.contrib.layers.fully_connected(F, 6, None)
...
def model(X_train, Y_train, X_test, Y_test, learning_rate = 0.009, num_epochs = 500, minibatch_size = 64, print_cost = True):
    """
    ...
    with tf.Session() as sess:
        ...
        save_path = saver.save(sess, "/home/madcas/Dropbox/ITM/Extensión/DeepLearning/Grupo02/Clase#4_RedesConvolucionales/model_softmax_Gestos4Capas_full.ckpt")
